...
XGBOOST_MODEL_PATH = './xgb_farmleaf.json'	# Path to the pretrained model
DATASET_FILEPATH = './farmdataset.csv'		# Path to the train/test dataset
...
for col in ['day', 'temperature', 'light', 'humidity', 'water','leaf','growth_cm','node_cm']:
    dataset[col] = pd.to_numeric(dataset[col], errors='coerce')

X, y = dataset.iloc[:, :-3], dataset.iloc[:, -3] 
...
for i in range(min(10000,len(test_X))):
	...
	print(f'leaf: {groundtruth:.1f}\tPred: {prediction:.1f}\tError: {error:.2f}%')
	# print('\n=====')
